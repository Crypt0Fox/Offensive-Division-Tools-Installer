...
sudo apt install -y dhclient curl wget git unzip python3 python3-pip python3-venv build-essential jq net-tools docker.io docker-compose pipx cmatrix lolcat figlet zsh fzf bat ripgrep fortune
...
cd /opt/recon && git clone https://github.com/Tib3rius/AutoRecon.git && cd AutoRecon && python3 -m venv venv && source venv/bin/activate && pip install -r requirements.txt && deactivate
cd /opt/active-directory && git clone https://github.com/BloodHoundAD/BloodHound.git BloodHoundCE && cd BloodHoundCE/docker && docker compose -f docker-compose.linux.yml up -d
...
# === 11. Reboot Countdown ===
echo -e "${ORANGE}[!] Rebooting in 60 seconds to finalize Docker group permissions.${NC}"
